...
class Application(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(
            "py打包exe工具V2.15 (python环境,pyinstaller模块开发）")  # 命令符：pip install pyinstaller   tkinter模块 pip install tkinter
        ...
        self.process_text.insert(tk.END, "1. python环境2.7以上,pyinstaller模块开发，可进行多模块打包；感谢Q同学支持； \n")
        ...
        self.process_text.insert(tk.END, "4. 如用代码运行打包其他代码时，无限制；如需把自身打包生成打包工具（建议打包成目录版），如需生"
                                         "成exe工具版需要复制tk tcl tcl8三个依赖（在打包工具目录版里有}到exe程序同目录下），在进行"
                                         "打包其他代码或模块； \n")
        self.process_text.insert(tk.END,
                                 "5. 注意必要依赖：tk tcl tcl8  三个文件包；默认已在打包工具目录版里了，可复制三个文件所在路径填写到"
                                 "指定含依赖路径栏里； \n")
        self.process_text.insert(tk.END, "...... \n")
    ...
    def generate_executable(self):
        ...
        with open(temp_file, "w") as f:
            ...
            for line in process.stdout:
                ...
                total_lines += 1
                progress_value = min(int((total_lines / 100) * 100), 95)  # 计算进度条的值
        ...
        if package_type == "目  录(-onedir)":
            ...
            if getattr(sys, 'frozen', False): # 打包后
                copy_status = self.copy_tcl_and_tk_folders(self.dep_entry.get(), output_directory)
                self.process_text.insert(tk.END, "分享者QQ:790544312！\n" "恭喜！生成完成！\n" if copy_status else "很遗憾，生成失败！")
                self.copy_resource_files(file_dir, output_directory)
    ...
    def copy_resource_files(self, source_dir, target_dir, exclude_dirs=['__pycache__']):
        if not os.path.exists(target_dir):
            self.process_text.insert(tk.END, "您还没有完成打包流程！\n")
            return

        # 排除自身
        exclude_dirs.append(os.path.basename(target_dir))
